...
loginForm.addEventListener('submit', (event) => {
  ...
  const aadhar_number = document.getElementById('aadhar-number').value;
  ...
  fetch(`http://127.0.0.1:8000/login?voter_id=${voter_id}&aadhar_number=${aadhar_number}&password=${password}`, { headers })
  .then(response => {
    if (response.ok) {
